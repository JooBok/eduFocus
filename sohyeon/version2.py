...
class BlinkDetector:
    ...
    def calculate_concentration(self, e_l, e_r, c_l, c_r, frame_number):
        ...
        if frame_number % 1000 == 0:
            print(f"Frame {frame_number}: e_l: {e_l}, e_r: {e_r}, c_l: {c_l}, c_r: {c_r}, e_c: {e_c}")
        ...
        blink_factor = 1 - min(1, self.blink_count / 100)
        concentration *= blink_factor
 
        if frame_number % 1000 == 0:
            print(f"Frame {frame_number}: Calculated concentration: {concentration}, Blink count: {self.blink_count}, Blink factor: {blink_factor}")
        ...
        return self.calculate_concentration_score(frame_number)

    def calculate_concentration_score(self, frame_number):
        ...
        if frame_number % 1000 == 0:
            print(f"Concentration score (before normalization): {concentration_score}")

        # 집중도 점수를 0과 1 사이로 제한
        normalized_score = max(0, min(concentration_score, 1.0))
        
        if frame_number % 1000 == 0:
            print(f"Concentration score (normalized): {normalized_score}")
...
class ConcentrationApp:
    ...
    def real_time_concentration_measurement(self):
        ...
        csv_file_path=os.path.join(save_path, f'concentration_data_{timestamp}.csv')
        detailed_csv_file_path=os.path.join(save_path, f'detailed_landmarks_{timestamp}.csv')
        ...
        # CSV 파일 생성 및 초기화
        with open(csv_file_path, mode='w', newline='') as file, open(detailed_csv_file_path, mode='w', newline='') as detailed_file:
