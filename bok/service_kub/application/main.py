    ...
    for input_dir in input_dirs:
        ...
        for filename in sorted(os.listdir(input_dir)):
            if filename.endswith('.jpg') or filename.endswith('.png'):  # 이미지 파일만 처리
                ...
                # saliency map을 바이너리 형식으로 변환
                saliency_map_binary = saliency_map.tobytes()

                # GridFS에 saliency map 저장
                try:
                    file_id = save_to_gridfs(db, collection_name, saliency_map_binary, filename, frame_number)
                    print(f"Processed and saved binary saliency map for frame {frame_number} from {filename} to collection {collection_name}")
                except Exception as e:
                    print(f"Failed to save binary saliency map for frame {frame_number} from {filename}: {e}")

                # 프레임 번호 증가
                frame_number += 1

        print(f"All binary saliency maps from {input_dir} have been saved to MongoDB in collection {collection_name}")